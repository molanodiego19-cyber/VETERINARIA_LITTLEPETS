...
from django import forms
from .models import Especialidad, HorarioVeterinario, BloqueoAgenda
from django.forms.widgets import DateInput, TimeInput, Select, CheckboxInput, Textarea
...
class HorarioVeterinarioForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'intervalo_min': forms.NumberInput(attrs={'class': 'form-control', 'min': 1, 'placeholder': 'Intervalo en minutos'}),
            'activo': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'fecha_desde': forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date', 'class': 'form-control'}),
            'fecha_hasta': forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date', 'class': 'form-control'}),
        }
...
class BloqueoAgendaForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'motivo': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Motivo del bloqueo'}),
