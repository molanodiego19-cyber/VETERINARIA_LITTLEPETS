...
class MascotaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'sexo','peso_kg','foto','esterilizacion','color'
        ]
        widgets = {
    ...
    def clean_color(self):
        ...
        if len(color) < 3:
            raise forms.ValidationError("El color debe tener mínimo 3 caracteres")
    ...
    def clean_peso_kg(self):
        ...
        if len(peso_str.replace('.', '').replace(',', '')) > 5:
            raise forms.ValidationError("El peso tiene demasiados dígitos")
        
        if peso % 1 == 0:
            raise forms.ValidationError("Debe ingresar decimales, ejemplo: 10.50")
    ...
    def clean(self):
        ...
        if especie and raza:
            if raza.tipo_especie != especie:
                raise forms.ValidationError("La raza no pertenece a la especie")
...
class PerfilVeterinarioForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'dias_laborales': forms.TextInput(attrs={
                'class': 'form-control',
    ...
    def clean_dias_laborales(self):
        ...
        lista_dias = [d.strip().lower() for d in dias.split(',')]
