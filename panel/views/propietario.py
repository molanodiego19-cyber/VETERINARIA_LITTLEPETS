...
from usuarios.forms import PropietarioCompletoForm, PropietarioUpdateForm
...
def filtrar_propietarios(request):
    ...
    propietarios = Propietario.objects.select_related("usuario").all().order_by("-id")

    nombre = request.GET.get("nombre")

    if nombre:
        propietarios = propietarios.filter(nombre__icontains=nombre)

    documento = request.GET.get("documento")

    if documento:
        propietarios = propietarios.filter(documento__icontains=documento)

    telefono = request.GET.get("telefono")

    if telefono:
        propietarios = propietarios.filter(telefono__icontains=telefono)

    ciudad = request.GET.get("ciudad")

    if ciudad:
        propietarios = propietarios.filter(ciudad__icontains=ciudad)

    estado = request.GET.get("estado")

    if estado:
        propietarios = propietarios.filter(usuario__estado=estado)
...
class PropietarioListView(ListView):
    ...
    def get_template_names(self):
        ...
        if rol == "recepcionista":
            return ["panel/propietario/list_recepcionista.html"]

        return ["panel/propietario/list.html"]

    def get_queryset(self):

        return filtrar_propietarios(self.request)

    def get_context_data(self, **kwargs):

        context = super().get_context_data(**kwargs)

        context["estados"] = [("activo", "Activo"), ("inactivo", "Inactivo")]
...
class PropietarioCreateView(CreateView):
    ...
    success_url = reverse_lazy("panel:panel_propietario_list")
    ...
    def get_template_names(self):
        ...
        if rol == "recepcionista":
            return ["panel/propietario/form_recepcionista.html"]

        return ["panel/propietario/form.html"]
