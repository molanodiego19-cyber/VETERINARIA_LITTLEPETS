...
def dashboard(request):
    ...
    # =====================================================
    citas_hoy_stats = Cita.objects.filter(fecha=hoy).aggregate(
        agendadas=Count("id"),
        activas=Count("id", filter=~Q(estado="cancelada")),
    ...
    # INGRESOS
    # =====================================================
    ingresos_dia = Factura.objects.filter(fecha_creacion__gte=hoy).aggregate(
        total=Sum("total")
    )["total"]
    ...
    ingresos_mes = (
        Factura.objects.filter(fecha_creacion__gte=inicio_mes).aggregate(
            total=Sum("total")
        )["total"]
        or 0
    )
    ...
    servicios_populares = Servicio.objects.annotate(
        cuanto=Count(
            "citas"
        )  # Asegúrate que el related_name en tu modelo Cita sea 'citas'
    ).order_by("-cuanto")[:5]

    # =====================================================
    # TASA NO SHOW
    # =====================================================
    total_historico = Cita.objects.exclude(estado="facturada").count()
    inasistencias = Cita.objects.filter(estado="cancelada").count()
    tasa_no_show = (
        ((inasistencias / total_historico) * 100) if total_historico > 0 else 0
    )

    # =====================================================
    # CLIENTES NUEVOS
    # =====================================================
    nuevos_clientes = Propietario.objects.filter(
        fecha_registro__gte=hace_30_dias
    ).count()

    # Data de Tablas requerida por tu HTML
    ultimas_mascotas = Mascota.objects.order_by("-id")[:5]
    citas_atendidas = Cita.objects.filter(estado="atendida").order_by(
        "-fecha", "-hora"
    )[:5]
...
def perfil_propietario(request):
    ...
    proximas_citas = Cita.objects.filter(
        mascota__propietario=propietario, fecha__gte=timezone.now().date()
    ).order_by("fecha", "hora")[:5]

    context = {
        "total_mascotas": total_mascotas,
        "total_citas": total_citas,
        "citas_atendidas": proximas_citas,  # Cambiado para que use la tabla del HTML común
        "mascotas": mascotas,
    }
