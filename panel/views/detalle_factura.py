...
class DetalleFacturaListView(ListView):
    ...
    def get_queryset(self):
        ...
        # PROPIETARIO
            ...
            qs = qs.filter(factura__cita__mascota__propietario__usuario=usuario)
        ...
        # VETERINARIO
            ...
            qs = qs.filter(factura__cita__veterinario__usuario=usuario)
...
def filtrar_detalles(request, queryset):
    ...
    if factura:
        ...
        queryset = queryset.filter(factura__numero_factura__icontains=factura)
...
def detalle_factura_pdf_view(request, pk):
    ...
    if usuario.rol == "propietario":
        ...
        permitido = detalle.factura.cita.mascota.propietario.usuario == usuario
        ...
        permitido = detalle.factura.cita.veterinario.usuario == usuario
    ...
    response["Content-Disposition"] = f'filename="detalle_{detalle.id}.pdf"'
...
def detalle_factura_pdf_list(request):
    ...
    # PROPIETARIO
        ...
        detalles = detalles.filter(factura__cita__mascota__propietario__usuario=usuario)

    # VETERINARIO
    elif usuario.rol == "veterinario":

        detalles = detalles.filter(factura__cita__veterinario__usuario=usuario)
    ...
    response["Content-Disposition"] = 'attachment; filename="detalles_factura.pdf"'
