...
def obtener_url_facturas(usuario):
    if usuario.rol == "recepcionista":
        return "panel:panel_factura_list_recepcionista"

    return "panel:panel_factura_list"


class FacturaListViewAdmin(ListView):
    model = Factura
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if usuario.rol != "admin":
            return redirect("panel:panel_factura_list_recepcionista")

        return super().dispatch(request, *args, **kwargs)


class FacturaListViewRecepcionista(ListView):
    model = Factura
    template_name = "panel/facturas/list_recepcionista.html"
    context_object_name = "facturas"
...
class FacturaListView(ListView):
    ...
    # ======================================
    # UTILIDAD REDIRECCIÓN SEGÚN ROL
    # ======================================
    ...
    def get_queryset(self):
        ...
        # PROPIETARIO
            ...
            qs = qs.filter(cita__mascota__propietario__usuario=usuario)
        ...
        # VETERINARIO
            ...
            qs = qs.filter(cita__veterinario__usuario=usuario)
    ...
    def get_context_data(self, **kwargs):
        ...
        usuario = Usuario.objects.get(id=self.request.session.get("usuario_id"))
...
class FacturaCreateView(CreateView):
    ...
    def get_success_url(self):
        usuario = Usuario.objects.get(id=self.request.session.get("usuario_id"))
        return reverse_lazy(obtener_url_facturas(usuario))
...
def factura_pdf_view(request, pk):
    ...
    factura = Factura.objects.select_related(
        "cita", "cita__mascota", "cita__veterinario"
    ).get(pk=pk)
    ...
    if usuario.rol == "propietario":
        ...
        permitido = factura.cita.veterinario.usuario == usuario
...
def factura_pdf_list(request):
    ...
    # VETERINARIO
        ...
        facturas = facturas.filter(cita__veterinario__usuario=usuario)
...
def filtrar_facturas(request, queryset):
    ...
    if dueño:
        ...
        queryset = queryset.filter(cita__mascota__propietario__nombre__icontains=dueño)
