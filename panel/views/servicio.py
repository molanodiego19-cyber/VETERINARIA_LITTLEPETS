...
def historial_servicio(request):
    ...
    servicios = (
        ...
        .filter(Q(factura__cita__estado="finalizada"))
        .values("servicio__id", "servicio__nombre")
        .annotate(total_cantidad=Count("id"), total_ingresos=Sum("total"))
        .order_by("-total_cantidad")
    )

    return render(
        request, "panel/servicio/servicios_finalizados.html", {"servicios": servicios}
    )
